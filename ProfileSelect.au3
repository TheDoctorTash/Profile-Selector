...
Func _MenuSelector()
	...
	GUICreate("Profile Selector v1.5 Rus", 300, 80)
	GUICtrlCreateLabel("Для начала, запустите с правами админа!", 10, 8)
	GUICtrlCreateLabel("Выберите нужный профиль для использования: ", 10, 28)
	$idBalance = GUICtrlCreateButton("Баланс", 8, 50, 50, 20)
	$idMining = GUICtrlCreateButton("Майнинг", 62, 50, 50, 20)
	$idGaming = GUICtrlCreateButton("Гейминг", 116, 50, 50, 20)
	$idTweaks = GUICtrlCreateButton("Твики", 188, 50, 50, 20)
	$idExit = GUICtrlCreateButton("Выход", 242, 50, 50, 20)
	GUISetState()
	Do
		$iMsg = GUIGetMsg()
		Select
			Case $iMsg = $idBalance
				...
				MsgBox($MB_SYSTEMMODAL, "Готово", "Сбалансированный профиль активирован.")
				Exit
			Case $iMsg = $idMining
				RunWait(@ScriptDir & '\dispwin.exe -Sl -d1 -c -I Profiles\Monitor.icm')
				_OptimizedProcesses()
				...
				MsgBox($MB_SYSTEMMODAL, "Готово", "Майнинг профиль активирован.")
				Exit
			Case $iMsg = $idGaming
				RunWait(@ScriptDir & '\dispwin.exe -Sl -d1 -c -I Profiles\Monitor.icm')
				_OptimizedProcesses()
				...
				MsgBox($MB_SYSTEMMODAL, "Готово", "Игровой профиль активирован.")
				Exit
			Case $iMsg = $idTweaks
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Пожалуйста подождите, настройки будут применены в скрытом режиме.")
				ShellExecuteWait(@ScriptDir & "\Tweaks.cmd","","","", @SW_HIDE)
				RunWait(@ScriptDir & "\SetTimerResolutionService.exe -install", "", @SW_HIDE)
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Установите патч для снятия ограничений драйвера Nvidia на частоту обновления и разрешения экрана.")
				RunWait(@ScriptDir & "\nvlddmkm-patcher.exe", "", @SW_MAXIMIZE)
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Настройте зажим Nvidia для преобразования цветов перед отправкой их на монитор с широкой гаммой.")
				RunWait(@ScriptDir & "\novideo_srgb.exe", "", @SW_MAXIMIZE)
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Выберите устройства с поддержкой режима MSI и установите их приоритет.")
				RunWait(@ScriptDir & "\MSI_util_v3.exe", "", @SW_MAXIMIZE)
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Выберите устройства, чтобы установить для них определенные ядра процессора.")
				RunWait(@ScriptDir & "\intPolicy_x64.exe", "", @SW_SHOW)
				MsgBox($MB_SYSTEMMODAL, "Инфо", "Настройте приложение, чтобы исправить троттлинг процессора.")
				RunWait(@ScriptDir & "\ThrottleStop.exe", "", @SW_SHOW)
				MsgBox($MB_SYSTEMMODAL, "Готово", "Твики были применены. Пожалуйста, перезагрузите компьютер.")
				Exit
		EndSelect
